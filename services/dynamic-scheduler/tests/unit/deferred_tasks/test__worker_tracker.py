...
async def __h_return_constant_integer() -> int:
    return 42


async def __h_sum_numbers(first: int, second: float) -> float:
    return first + second


async def __h_stringify(name: str, age: int) -> str:
    return f"{name} is {age} years old"


async def __h_do_nothing(**kwargs) -> None:
    _ = kwargs
